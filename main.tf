...
resource "aws_instance" "app" {
  ...
  user_data = <<-EOF
    ...
    echo "<html><body><div>Hello, world 2!</div></body></html>" > /var/www/html/index.html
    EOF
...
#   count = length(aws_instance.app)
#
#   target_group_arn = data.terraform_remote_state.network.outputs.lb_target_group_https_arn
#   target_id        = aws_instance.app[count.index].id
#   port             = 443
# }
